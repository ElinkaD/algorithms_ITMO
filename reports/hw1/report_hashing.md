...
основное время уходит в Build → buildSecondary. Узкое место связано не столько с вычислением хэшей, сколько с большим числом аллокаций при построении структуры. 

в Build создаются вспомогательные структуры (map для проверки дублей и массив первичных bucket-ов), а в buildSecondary для каждого bucket-а с коллизиями создаётся генератор случайных чисел и на каждой попытке заново выделяется массив slots. Из-за большого количества временных объектов заметна работа сборщика мусора (runtime.gcBgMarkWorker, runtime.gcDrain). 

![Perfect Memory Framegraph](../../graphs/hw1/perfect/perfect_mem_100k_build.png.png)

Гипотезы по улучшению:

- в buildSecondary на каждой попытке создаётся новый slots → много аллокаций → нагрузка на GC
→ можно переиспользовать массив или чистить его вместо пересоздания
